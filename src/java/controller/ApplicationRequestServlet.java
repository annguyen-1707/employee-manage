...
public class ApplicationRequestServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String status = "In progress"; // Set default status
        ...
        try {
            ...
            Application application = new Application(title, startDate, endDate, reason, status,
                    employee.getEmployeeId());
            application = adao.insert(application);
            // Redirect with applicationId as parameter
            response.sendRedirect("applicationDetail?applicationId=" + application.getApplicationId());
        } catch (Exception e) {
            // Handle error and redirect back to form
            response.sendRedirect("applicationRequest?error=1");
        }
    }
